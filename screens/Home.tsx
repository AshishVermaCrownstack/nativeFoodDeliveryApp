...
const Home: FC<{navigation: any}> = ({navigation}) => {
  ...
  const [categories, setCategories] = useState<any[]>(categoryData);
  ...
  const onSelectCategory = (item: any) => {
    ...
    let restaurantList: any[] = restaurantData.filter(i =>
      i.categories.includes(item.id),
    );
  ...
  const getCategoryNameById = (categoryId: number) => {
    let restaurantCategoryList: any[] = categories.filter(
      i => i.id == categoryId,
    );
    if (restaurantCategoryList.length > 0) {
      return restaurantCategoryList.map(a => a.name);
    } else {
      return '';
    }
  };
